    ...
    init(from decoder: Decoder) throws {
        ...
        if let timestamp = try? container.decode(Timestamp.self, forKey: .date) {
            ...
            throw DecodingError.dataCorruptedError(
                forKey: .date,
                in: container,
                debugDescription: "Unsupported date representation."
            )
        }
        
        if let timestamp = try? container.decode(Timestamp.self, forKey: .end) {
            end = timestamp.dateValue()
        } else if let endValue = try? container.decode(Date.self, forKey: .end) {
            end = endValue
        } else {
            throw DecodingError.dataCorruptedError(
                forKey: .end,
                in: container,
                debugDescription: "Unsupported end date representation."
            )
        }
    }
}
