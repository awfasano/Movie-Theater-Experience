...
enum WindowType: String, CaseIterable {
    ...
    // Movie Theatre Experience Windows
...
class WindowManager: ObservableObject {
    ...
    private let availableBrowserInstances = ["1", "2"]
    ...
    /// Legacy method for call sites that manually track window open.
    /// With the new WindowTrackingModifier, this is handled automatically,
    /// but these wrappers prevent compilation errors in existing code.
    func windowOpened(_ type: WindowType) {
        trackWindow(type)
    }

    /// Legacy method for call sites that manually track window close.
