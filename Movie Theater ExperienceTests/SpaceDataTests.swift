...
class SpaceDataTests: XCTestCase {
    ...
    func testCodable() throws {
        let date = Date()
        let space = SpaceData(id: "2", spaceName: "Another Space", description: "", lastModified: date, usdzURL: "", thumbnailURL: nil, mapURL: nil, attributions: nil, tags: nil, introEntityName: nil, currentSeat: nil, viewerXAdjustment: 0, viewerYAdjustment: 0, viewerZAdjustment: 0, currentUserCount: 5, maxUserCount: 10, mapImageURL: nil, seats: nil)
        ...
        let decodedSpace = try decoder.decode(SpaceData.self, from: data)
        
        XCTAssertEqual(space.id, decodedSpace.id)
        XCTAssertEqual(space.spaceName, decodedSpace.spaceName)
    }
